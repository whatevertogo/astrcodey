...
3. `PromptRequest` → `handle_prompt()` 提取文本 → `CommandHandle::submit_prompt_for_session()` 提交 turn
4. 通过 `ActiveTurnTracker` 等待 turn 完成（oneshot channel），同时从 broadcast channel 转发事件为 ACP `SessionNotification`
5. turn 完成后 drain 残留事件（100ms 窗口），返回 `PromptResponse`（含 `StopReason`）
...
- **`ActiveTurnTracker`**：基于 `DashMap<(SessionId, TurnId), oneshot::Sender>` 的 turn 追踪器，支持 register / resolve / cancel_session
- **`TurnOutcome`**：枚举 `Completed { stop_reason }`，唯一变体
