//! `ContextSnapshot` 构建与请求组装的微基准（PR-1:消息体 Arc 共享化）。
//!
...
/// 模拟含工具结果的真实 transcript 单条消息量级。
const MESSAGE_BODY_CHARS: usize = 2_000;

fn sample_transcript() -> Vec<TranscriptMessage> {
    (0..MESSAGE_COUNT)
        .map(|index| {
            let body = format!("{:width$}", index, width = MESSAGE_BODY_CHARS);
