...
//!
//! 进程级路径原语（`user_home_dir` / `astrcode_dir` / `extension_data_dir`）
//! 由 [`astrcode_paths`] 提供，这里为保持 `config::defaults` 作为逻辑归属地
//! 继续 re-export。

pub use astrcode_paths::{astrcode_dir, extension_data_dir, user_home_dir};
...
/// LLM 最大重试次数。
pub const DEFAULT_LLM_MAX_RETRIES: u32 = 5;
/// LLM 重试的指数退避基础延迟（毫秒）。
pub const DEFAULT_LLM_RETRY_BASE_DELAY_MS: u64 = 1_000;
/// 模型未显式配置 `maxTokens` 时的默认值。
pub const DEFAULT_LLM_MAX_TOKENS: u32 = 8192;
/// 模型未显式配置 `contextLimit` 时的默认值。
pub const DEFAULT_LLM_CONTEXT_LIMIT: usize = 65536;
