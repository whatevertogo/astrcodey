...
use crate::router::{HandlerError, PromptSubmission};
...
/// 解析斜杠命令，如 "/compact arg1 arg2"。返回 None 表示不是斜杠命令。
...
pub(crate) fn command_error_code(error: &HandlerError) -> i32 {
    match error {
...
pub(crate) async fn command_infos_for_working_dir(
    ...
    let mut extension_commands = extension_runner
...
impl SessionActor {
    /// 执行指定会话的斜杠命令。
    pub(crate) async fn execute_slash_command_for_session(
        &mut self,
        ...
        if command.name == "compact" {
            return match self.compact_session(&sid).await? {
                crate::session::ManualCompactOutcome::Compacted { .. } => {
                    Ok(PromptSubmission::Handled {
                        message: "compact accepted".into(),
                    })
                },
                crate::session::ManualCompactOutcome::Skipped { message } => {
