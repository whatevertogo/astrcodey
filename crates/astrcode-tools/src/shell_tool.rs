...
fn shell_tool_definition(timeout_secs: u64) -> ToolDefinition {
    ...
    let definition = ToolDefinition {
        ...
        description: format!(
            concat!(
                "Executes a {shell} command and returns its output. The working directory ",
                ...
                "- Use `stdin` to pipe data into the command (e.g., for `jq`, `wc`, `python`).\n",
                "- Non-zero exit codes produce an error result.\n",
