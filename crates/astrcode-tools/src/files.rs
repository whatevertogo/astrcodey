...
impl Tool for EditFileTool {
    fn definition(&self) -> ToolDefinition {
        ToolDefinition {
            name: "editFile".into(),
            description: "Replace exactly one occurrence of old_string with new_string. Add \
    ...
    async fn execute(&self, args: serde_json::Value) -> Result<ToolResult, ToolError> {
        ...
        let path = resolve_path(&self.working_dir, raw);
        if old_str.is_empty() {
            return Err(ToolError::InvalidArguments("old_string empty".into()));
        }

        let original = clean_quotes(
        ...
        if count > 1 {
            return Err(ToolError::Execution(format!(
                "old_string found {count} times — add context"
            )));
        }
...
impl Tool for GrepTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> Result<ToolResult, ToolError> {
        ...
        let root = if let Some(p) = args["path"].as_str() {
            resolve_path(&self.working_dir, p)
        } else {
            self.working_dir.clone()
        };
        let mut matches = Vec::new();
        walk_grep(&root, &re, &mut matches, 250)
...
fn walk_grep(
    ...
    if root.is_file() {
        ...
        for e in std::fs::read_dir(root)? {
            let p = e?.path();
            if !(p.is_dir() && p.file_name().map_or(false, |n| n == ".git")) {
                walk_grep(&p, re, out, max)?;
            }
        }
    }
    Ok(())
