...
| `astrcode-extension-sdk` | - | 扩展公共 API、能力声明与状态命名空间入口 |
| `astrcode-extensions` | 2.8k | 扩展生命周期、能力门控、钩子分发、WASM 扩展运行时 |
| `astrcode-extension-mcp` | ~2.4k | MCP 协议客户端 — 常驻进程池、后台预热、并发合并、健康检查 |
...
| `astrcode-extension-memory` | ~1.8k  | 项目作用域的 Markdown 记忆存储 |
...
**共计：20 个 Rust crate + Tauri shell、203 个源文件、约 5.7 万行代码。**

### 前端与桌面应用

| 组件 | 行数 | 说明 |
|---|---|---|
| `frontend/`（React + TS） | ~4.8k | Web 前端——聊天视图、侧边栏、会话管理、SSE 流式传输、状态栏 |
...
- **WASM 扩展运行时** — 基于 wasmtime 的沙箱 + **s5r 对称 peer**（`peer_exchange` / `handler.invoke` / `astrcode.*`）；磁盘扩展须在 `extension.json` 中设置 `protocol.s5r: "1.0"`。规范见 [docs/extension-system.md](docs/extension-system.md)
