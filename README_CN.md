...
可通过 `~/.astrcode/config.json` 的 `extensionStates` 字段启用或禁用扩展。默认情况下，除 `memory` 与 `channels` 扩展外，所有扩展均处于启用状态。
...
要启用 channels 扩展，在 `extensionStates` 中添加 `"astrcode-channels": true`，并在 `extensions` 中配置 Telegram 通道。
...
完整选项见[配置指南](docs/configuration.md#web-tools-extension)。
...
Cargo workspace 在 [`crates/`](crates/) 下包含 **23 个 crate**，另有 [`src-tauri/`](src-tauri/) 作为桌面壳（workspace 共 **24 个成员**）。按架构分层如下（详见[架构设计](docs/architecture.md)）。

### Layer 0：基础层

| Crate | 行数 | 说明 |
|---|---|---|
| [`astrcode-core`](crates/astrcode-core) | 6.7k | 共享领域类型、trait、配置系统、扩展契约、提示词组合 |
| [`astrcode-support`](crates/astrcode-support) | 1.1k | 宿主工具：路径解析、Shell 检测、工具结果持久化 |
| [`astrcode-log`](crates/astrcode-log) | 0.3k | 文件轮转、stderr 输出、env-filter 日志 |

### Layer 1：领域服务层

| Crate | 行数 | 说明 |
|---|---|---|
| [`astrcode-ai`](crates/astrcode-ai) | 4.0k | 多 Provider LLM 层（Anthropic、OpenAI 兼容、Google GenAI）、SSE 流式、重试 |
| [`astrcode-tools`](crates/astrcode-tools) | 6.2k | 内置工具：read、write、edit、patch、glob、grep、shell、terminal、task |
| [`astrcode-storage`](crates/astrcode-storage) | 4.0k | JSONL 事件日志、快照、配置持久化、文件锁 |
| [`astrcode-context`](crates/astrcode-context) | 3.8k | Token 估算、上下文窗口预算、自动压缩、提示词引擎 |
| [`astrcode-session`](crates/astrcode-session) | 9.0k | Agent 循环：turn runner、工具管线、LLM 流、压缩编排、运行时服务 |
| [`astrcode-extensions`](crates/astrcode-extensions) | 5.2k | 扩展生命周期、钩子分发、能力门控、磁盘 IPC 扩展加载 |

### Layer 2：扩展层

| Crate | 行数 | 说明 |
|---|---|---|
| [`astrcode-extension-sdk`](crates/astrcode-extension-sdk) | 2.3k | 扩展作者稳定 API、能力声明、线缆协议类型、manifest 辅助 |
| [`astrcode-bundled-extensions`](crates/astrcode-bundled-extensions) | 0.1k | 组合根：注册全部第一方扩展 crate |
| [`astrcode-extension-mode`](crates/astrcode-extension-mode) | 1.2k | Code / Plan 模式切换、Exit Gate、计划 Artifact、快捷键与状态栏 |
| [`astrcode-extension-skill`](crates/astrcode-extension-skill) | 0.9k | 斜杠命令技能发现与 Skill 工具调度 |
| [`astrcode-extension-todo-tool`](crates/astrcode-extension-todo-tool) | 0.8k | 进度追踪 Todo 工具 |
| [`astrcode-extension-agent-tools`](crates/astrcode-extension-agent-tools) | 0.7k | 子 Agent 委派、Agent 发现（兼容 Claude Code 格式） |
| [`astrcode-extension-mcp`](crates/astrcode-extension-mcp) | 2.8k | MCP 客户端：stdio/HTTP 传输、常驻进程池、预热、健康检查 |
| [`astrcode-extension-memory`](crates/astrcode-extension-memory) | 2.8k | 项目作用域 Markdown 记忆（默认关闭） |
| [`astrcode-extension-channels`](crates/astrcode-extension-channels) | 1.0k | Telegram 通道桥接（默认关闭） |
| [`astrcode-extension-web-tools`](crates/astrcode-extension-web-tools) | 1.8k | Web 搜索与 URL 抓取工具（含 SSRF 防护与抓取缓存） |
...
| [`astrcode-server`](crates/astrcode-server) | 13.7k | 会话管理、JSON-RPC/HTTP/ACP、transport、HTTP 投影与 SSE |

### Layer 4：客户端层

| Crate | 行数 | 说明 |
|---|---|---|
| [`astrcode-client`](crates/astrcode-client) | 0.6k | 类型化 JSON-RPC 客户端、传输抽象、流订阅 |
| [`astrcode-cli`](crates/astrcode-cli) | 7.3k | CLI 入口：TUI（ratatui）、无头 exec、server 启动器 |
...
| 组件 | 行数 | 说明 |
|---|---|---|
| [`src-tauri/`](src-tauri) | 0.7k | Tauri v2 壳：sidecar 管理、单实例协调、原生对话框 |

**合计：** Rust 约 7.95 万行（23 个 crate + Tauri），**302** 个 `.rs` 文件；`frontend/` 约 8600 行 TypeScript（整体约 **8.82 万行**）。
