...
| [`astrcode-storage`](crates/astrcode-storage) | 4.3k | JSONL 事件日志、快照、`replay_events_through`、配置持久化、文件锁 |
| [`astrcode-context`](crates/astrcode-context) | 4.0k | Token 估算、上下文窗口预算、自动压缩、提示词引擎 |
| [`astrcode-session`](crates/astrcode-session) | 8.9k | Agent 循环：`Session::submit`、turn runner、工具管线、LLM 流、compact、`ChildTurnGuard` |
...
| Crate | 行数 | 说明 |
|---|---|---|
| [`astrcode-extension-sdk`](crates/astrcode-extension-sdk) | 2.5k | 扩展作者稳定 API、能力声明、线缆协议类型、manifest 辅助 |
...
| Crate | 行数 | 说明 |
|---|---|---|
| [`astrcode-protocol`](crates/astrcode-protocol) | 1.7k | 进程内命令/事件、HTTP/SSE 线缆 DTO |
| [`astrcode-server`](crates/astrcode-server) | 13.5k | `TurnScheduler` / `TurnRegistry`、会话管理、HTTP/SSE、ACP、`ServerSessionOperations`、Actor |

### Layer 4：客户端层

| Crate | 行数 | 说明 |
|---|---|---|
| [`astrcode-client`](crates/astrcode-client) | 693 | 类型化进程内客户端、传输抽象、流订阅 |
...
Web 前端（`frontend/`）是 React 19 + TypeScript + Tailwind CSS v4 + Vite 单页应用，通过 SSE 实时接收流式事件，通过 REST 发送命令。支持浏览器独立运行（`npm run dev`）或打包为 Tauri 桌面应用（`npm run tauri:dev`）。
...
ACP 适配器（`astrcode-server::acp`）将标准 Agent Client Protocol 桥接到 astrcode 内部的命令/广播架构，经 **HTTP WebSocket**（`GET /api/acp/ws`，与 REST 共用 Bearer 鉴权）。

仅支持 stdio 子进程的 IDE 可使用 **`astrcode acp` 桥**（`astrcode-cli::acp_bridge`）：不持有 server 状态，仅在 stdin/stdout 与本地 WebSocket 之间转发 JSON-RPC（默认读 `~/.astrcode/run.json`）。

- WebSocket 文本帧上的 JSON-RPC（每帧一条消息）
- 通过 broadcast channel 实时流式转发事件为 ACP `SessionNotification`
- Turn 完成由 `TurnScheduler` completion watcher 处理；扩展同步路径使用 `submit_and_wait`（`TurnSummary`）
- 为 IDE 插件和编辑器集成设计
...
- **Session 是投影** — 从事件日志重放重建状态；fork 使用 cursor 截断重放（`replay_events_through`）
- **Agent 无状态** — `TurnRunner` 每回合后丢弃；状态存在于事件日志中
- **恢复即重放** — Agent 崩溃不影响会话完整性；重新投影即可继续

### Turn 调度（`astrcode-server`）

编排层在 **`TurnScheduler`**，不在 agent 循环内：

- **`TurnRegistry`** — 进程内唯一索引：该 session 是否有 turn 在执行（`has_active_turn` API 仅看 registry）
- **`pending_queues`** — turn 运行中连发 prompt 的唯一 FIFO 队列；由内部 completion watcher 链在 turn 结束后出队
- **Handler / HTTP** — `accept_user_input` → scheduler；调用方不再手动 spawn completion watcher
- **扩展 API** — `ServerSessionOperations::submit_turn` 使用 `submit_tracked`、`submit_and_wait` 或 `submit_untracked`（子 session 配合 `ChildTurnGuard`）
- **Session idle** — 整条 turn 链结束后 Actor 收到 `SessionTurnIdle`，可启动 5 分钟 auto-recap

详见 [docs/architecture.md](docs/architecture.md) 第 2 节。
...
| **Server** | `cargo run -- server [--addr 0.0.0.0:3847]` | HTTP/SSE 服务器 + ACP WebSocket（`/api/acp/ws`） |
| **ACP 桥** | `cargo run -- acp` | Stdio JSON-RPC 转发到本地 server WebSocket（兼容 IDE 子进程） |
...
- **[OpenCode](https://github.com/anomalyco/opencode)** — 前后端分离（HTTP/SSE + REST）参考了 OpenCode 的架构。
