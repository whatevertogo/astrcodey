...
详细的配置文档请参阅[配置指南](docs/configuration_cn.md)。
...
Cargo workspace 在 [`crates/`](crates/) 下包含 **21 个 crate**，另有 [`src-tauri/`](src-tauri/) 作为桌面壳（workspace 共 **22 个成员**）。按架构分层如下（详见[项目结构](docs/Project-Structure.md)）。
...
| [`astrcode-extensions`](crates/astrcode-extensions) | 5.1k | 扩展生命周期、钩子分发、能力门控、WASM 运行时（wasmtime + s5r） |

### Layer 2：扩展层

| Crate | 行数 | 说明 |
|---|---|---|
| [`astrcode-extension-sdk`](crates/astrcode-extension-sdk) | 642 | 扩展作者稳定 API、能力声明、s5r 线协议类型、manifest 辅助 |
...
- **能力声明** — 内置扩展通过 `Extension::capabilities()`；WASM 扩展在 s5r 握手（`extension_init`）的 `requested_capabilities` 中声明 `session_state`、`session_control`、`small_model` 等；运行时经 `HostRouter` 鉴权后仅允许已声明的 `astrcode.*` invoke
...
- **WASM 扩展运行时** — 基于 wasmtime 的沙箱 + **s5r 对称 peer**（`peer_exchange` / `handler.invoke` / 按能力裁剪的 `astrcode.*` 宿主调用、多步 **continuations**）；磁盘扩展须在 `extension.json` 中设置 `protocol.s5r: "1.0"`。规范见 [docs/extension-system.md](docs/extension-system.md)
...
| 文档 | 说明 |
|---|---|
| [项目结构](docs/Project-Structure.md) | Workspace 布局、crate 分层、HTTP/前端/Tauri 模块 |
| [设计概述](docs/AstrCode-Design.md) | 事件溯源、压缩、工具、扩展、评测 |
| [配置指南](docs/configuration_cn.md) | 完整 `config.json` 参考 |
| [扩展系统](docs/extension-system.md) | 内置与 WASM 扩展、s5r 协议、宿主能力 |
| [Session-First](docs/session-first.md) | 事件日志生命周期与重放模型 |
