...
| Crate | 行数 | 说明 |
|---|---|---|
| [`astrcode-kernel`](crates/astrcode-kernel) | 0.5k | 可嵌入 kernel：工具包组合、工具注册表、扩展运行时抽象 |
| [`astrcode-core`](crates/astrcode-core) | 9.1k | 共享领域类型、trait、配置系统、扩展契约、提示词组合 |
| [`astrcode-support`](crates/astrcode-support) | 1.3k | 宿主工具：路径解析、Shell 检测、工具结果持久化 |
| [`astrcode-log`](crates/astrcode-log) | 0.4k | 文件轮转、stderr 输出、env-filter 日志 |

### Layer 1：领域服务层

| Crate | 行数 | 说明 |
|---|---|---|
| [`astrcode-ai`](crates/astrcode-ai) | 5.2k | 多 Provider LLM 层（Anthropic、OpenAI 兼容、Google GenAI）、SSE 流式、重试 |
| [`astrcode-tools`](crates/astrcode-tools) | 7.8k | 内置工具：read、write、edit、patch、glob、grep、shell、terminal、task |
| [`astrcode-storage`](crates/astrcode-storage) | 4.6k | JSONL 事件日志、快照、配置持久化、文件锁 |
| [`astrcode-context`](crates/astrcode-context) | 4.5k | Token 估算、上下文窗口预算、自动压缩、提示词引擎 |
| [`astrcode-session`](crates/astrcode-session) | 11.3k | Agent 循环：turn runner、工具管线、LLM 流、压缩编排、运行时服务 |
| [`astrcode-extensions`](crates/astrcode-extensions) | 7.4k | 扩展生命周期、钩子分发、能力门控、磁盘 IPC 扩展加载 |

### Layer 2：扩展层

| Crate | 行数 | 说明 |
|---|---|---|
| [`astrcode-extension-sdk`](crates/astrcode-extension-sdk) | 2.7k | 扩展作者稳定 API、能力声明、线缆协议类型、manifest 辅助 |
| [`astrcode-bundled-extensions`](crates/astrcode-bundled-extensions) | 0.2k | 组合根：注册全部第一方扩展 crate |
| [`astrcode-extension-mode`](crates/astrcode-extension-mode) | 1.3k | Code / Plan 模式切换、Exit Gate、计划 Artifact、快捷键与状态栏 |
| [`astrcode-extension-goal`](crates/astrcode-extension-goal) | 1.6k | Codex 风格会话目标、Token 预算、自动延续 |
| [`astrcode-extension-skill`](crates/astrcode-extension-skill) | 1.0k | 斜杠命令技能发现与 Skill 工具调度 |
| [`astrcode-extension-todo-tool`](crates/astrcode-extension-todo-tool) | 0.9k | 进度追踪 Todo 工具 |
| [`astrcode-extension-agent-tools`](crates/astrcode-extension-agent-tools) | 0.8k | 子 Agent 委派、Agent 发现（兼容 Claude Code 格式） |
| [`astrcode-extension-mcp`](crates/astrcode-extension-mcp) | 3.4k | MCP 客户端：stdio/HTTP 传输、常驻进程池、预热、健康检查 |
| [`astrcode-extension-memory`](crates/astrcode-extension-memory) | 3.2k | 项目作用域 Markdown 记忆（默认关闭） |
| [`astrcode-extension-channels`](crates/astrcode-extension-channels) | 1.1k | Telegram 通道桥接（默认关闭） |
| [`astrcode-extension-web-tools`](crates/astrcode-extension-web-tools) | 1.9k | Web 搜索与 URL 抓取工具（含 SSRF 防护与抓取缓存） |

### Layer 3：服务与协议层

| Crate | 行数 | 说明 |
|---|---|---|
| [`astrcode-protocol`](crates/astrcode-protocol) | 1.8k | JSON-RPC 2.0 线协议类型、命令、事件、HTTP/UI DTO |
| [`astrcode-server`](crates/astrcode-server) | 17.3k | 会话管理、JSON-RPC/HTTP/ACP、transport、HTTP 投影与 SSE |

### Layer 4：客户端层

| Crate | 行数 | 说明 |
|---|---|---|
| [`astrcode-client`](crates/astrcode-client) | 0.7k | 类型化 JSON-RPC 客户端、传输抽象、流订阅 |
...
| Crate | 行数 | 说明 |
|---|---|---|
| [`astrcode-eval`](crates/astrcode-eval) | 1.1k | 评测运行器：HTTP 服务器控制、事件日志指标、结构化报告 |

### 桌面壳

| 组件 | 行数 | 说明 |
|---|---|---|
| [`src-tauri/`](src-tauri) | 0.8k | Tauri v2 壳：sidecar 管理、单实例协调、原生对话框 |

**合计：** Rust 约 10 万行（26 个 crate + Tauri），**317** 个 `.rs` 文件；`frontend/` 约 1.14 万行 TypeScript（整体约 **11.1 万行**）。
