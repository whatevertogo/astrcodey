...
- **磁盘 IPC 扩展** — stdio JSON-RPC / JSONL 子进程（`extension.json` 中 `protocol.ipc` + `command`）；`extension/initialize`、`extension/handler.invoke` 与按能力裁剪的 `host/invoke`。规范见 [docs/extension-system.md](docs/extension-system.md)
