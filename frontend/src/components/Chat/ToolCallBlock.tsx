...
import { pillNeutral, pillSuccess, pillDanger, chevronIcon, codeBlockShell, codeBlockContent } from '../../lib/styles';
...
function statusPill(status: string): string {
  switch (status) {
    case 'complete': return pillSuccess;
    case 'error': return pillDanger;
...
function ToolCallBlock({ block }: ToolCallBlockProps) {
  ...
  const displayText = block.text || (block.status === 'streaming' ? '等待输出...' : '');
  ...
  return (
    ...
    >
      <summary className="flex min-w-0 cursor-pointer items-center gap-2 py-1.5 font-mono text-[13px] leading-relaxed text-text-secondary list-none [&::-webkit-details-marker]:hidden hover:opacity-85">
        <span className={cn('shrink-0', statusPill(block.status))}>{block.name}</span>
        <span className="min-w-0 flex-1 truncate text-text-primary">{displayText.slice(0, 100)}</span>
        <span className="shrink-0 text-text-muted">{statusLabel(block.status)}</span>
