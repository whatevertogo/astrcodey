...
import { assistantAvatar, codeBlockShell, codeBlockHeader, codeBlockContent, ghostIconButton } from '../../lib/styles';
...
class MarkdownGuard extends Component<{ fallback: string; children: React.ReactNode }, { hasError: boolean }> {
  state = { hasError: false };
  static getDerivedStateFromError() { return { hasError: true }; }
  render() {
    if (this.state.hasError) {
      return <pre className="m-0 whitespace-pre-wrap overflow-wrap-anywhere font-inherit text-inherit">{this.props.fallback}</pre>;
    }
...
function CopyButton({ code }: { code: string }) {
  ...
  return (
    <button
      className={cn(ghostIconButton, 'h-7 gap-1.5 rounded px-2 text-[13px] opacity-0 translate-y-0.5 group-hover:translate-y-0 group-hover:opacity-100')}
      onClick={handleCopy}
      title="复制代码"
    >
      {copied ? (
        <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg><span>已复制</span></>
      ) : (
        <><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg><span>复制</span></>
      )}
    </button>
  );
}
...
const markdownComponents = {
  ...
  code: CodeBlockRenderer as React.ComponentType<React.ComponentPropsWithoutRef<'code'>>,
};

const MarkdownContent = memo(function MarkdownContent({ text, defer = false }: { text: string; defer?: boolean }) {
...
function extractThinkingBlocks(text: string): { visibleText: string; thinkingBlocks: string[] } {
  const thinkingBlocks: string[] = [];
  const visibleText = text.replace(/<think([\s\S]*?)<\/think>/gi, (_match, content: string) => {
    ...
    <div className="flex items-start gap-4 animate-message-enter max-sm:gap-3 motion-reduce:animate-none">
      <div className={assistantAvatar} aria-hidden="true">
        <svg viewBox="0 0 20 20" className="w-4 h-4">
          <rect x="3.25" y="3.25" width="13.5" height="13.5" rx="3.5" fill="none" stroke="currentColor" strokeWidth="1.4" />
          <path d="M8 8h4M8 10h4M8 12h2.5" fill="none" stroke="currentColor" strokeLinecap="round" strokeWidth="1.4" />
        </svg>
      </div>
      <div className="min-w-0 flex-1 pt-0.5">
        <div className="relative min-w-0 max-w-full overflow-wrap-anywhere bg-transparent py-2 text-text-primary prose-chat">
          {thinkingBlocks.map((block, index) => (
            <details key={`thinking-${index}`} className="mb-3.5 bg-transparent border-none rounded-0 overflow-visible group" open={streaming}>
