...
import { composerShell, composerSubmitButton, composerInterruptButton } from '../../lib/styles';
import { cn } from '../../lib/utils';

function isExecutionPhase(phase: string): boolean {
  return phase === 'thinking' || phase === 'streaming' || phase === 'calling_tool';
}
      ...
      <div className="mx-auto w-full max-w-[var(--chat-composer-max-width)] translate-x-[var(--chat-assistant-center-shift)]">
        <div className="relative w-full">
          <div className={composerShell}>
            {workingDir && (
              <div className="flex items-center gap-2 rounded-t-[23px] border-b border-border bg-white/40 px-4 py-2.5 text-text-secondary" title={workingDir}>
                <span className="inline-flex h-3.5 w-3.5 flex-shrink-0 items-center justify-center" aria-hidden="true">
                ...
                <div className="overflow-hidden text-ellipsis whitespace-nowrap font-mono text-xs">{workingDir}</div>
              </div>
            )}
            <div className="relative">
              <div className="flex flex-col px-[var(--chat-composer-shell-padding-x)] py-3">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex flex-shrink-0 items-center gap-2">
                    {isBusy ? (
                      <button className={composerInterruptButton} type="button" onClick={() => void abortCurrentTurn()}>
                        中断
                      </button>
                    ) : (
                      <button
