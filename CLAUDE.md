...
- 避免用 utils / helper / manager 掩盖职责；只有确实表达资源生命周期或协调职责时才使用 manager
...
- 只有测试能放 `.unwrap()`。
遵循 Rust 语境下的职责分离：模块负责边界，类型表达约束；封闭集合优先 enum，开放扩展优先 trait；只有存在多个实现、测试替身或跨层边界时才引入 trait，不为抽象而抽象。
