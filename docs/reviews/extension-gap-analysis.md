...
### P0 — 地基已打好,暴露面即可(成本低收益大)

| # | 能力 | 现状与证据 | 补法 |
|---|------|-----------|------|
| 1 | `agent.inject`(排队不唤醒)/ `followup`(排队并唤醒) | host 有原语 `QueueIfRunningElseStart`(astrcode-server/src/turn_scheduler.rs:123),只暴露给 HTTP/ACP(session_command_service.rs:104),扩展用不了 | 把它加进 session_control wire 协议的 HostOperation 列表(client.rs:163-175) |
| 2 | 多语言扩展 SDK | s5r 协议独立成文(docs/s5r-protocol.md,146 行)+ conformance 二进制(crates/astrcode-extension-sdk/src/bin/s5r-conformance.rs),但只有 Rust worker SDK | 写薄 Python/TS SDK,拿 conformance harness 验收 |
| 3 | 工具 UI 呈现 intent(deepseek `presentCall/presentResult`) | 前端/TUI 都有 renderer 注册表但只收内置(frontend/src/components/Chat/toolRendererRegistry.tsx:29;crates/astrcode-cli 的 tui/ext/builtin.rs:555),`ToolDefinition` 无 UI 字段(tool.rs:41-58) | 把现有 `RenderSpec`(frontend/src/types/render-spec.ts:23)模型跨扩展边界暴露 |
| 4 | per-tool `timeoutMs` | 只有 host 全局超时(s5r INVOKE_TIMEOUT;runner/mod.rs:1407) | ToolDefinition 加字段,调度器读取 |
| 5 | `deferContext`(工具执行中追加 user message) | 可组合:声明 SessionControl + inject_or_start 走 Injected 分支(turn_scheduler.rs:120),但非一等 API 且语义耦合 delivery 策略 | ToolContext 加 `defer_context`,host 侧固定走注入语义 |
...
| 20 | 外部 hooks 桥(Claude Code/Codex hooks.json) | 无任何解析;兼容只到 agent 定义 frontmatter | 纯生态 shim,读旧配置跑 shell 映射到现有 hook 点 |
...
| 22 | **OS 级 sandbox**(landlock/seatbelt) | 只有策略级圈禁(路径 canonicalize + network 域名/凭据限制) | 跨平台复杂;明确有不可信扩展场景再做 |
| 23 | **PTY terminals** | 无 portable-pty 依赖,持久进程走管道 | 交互式终端程序的前提 |
