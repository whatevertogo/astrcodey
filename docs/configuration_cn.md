...
- **扩展数据**: `~/.astrcode/extensions_data/astrcode.memory/`
...
扩展可以在 `~/.astrcode/extensions_data/<extension-id>/` 中存储自己的数据。

例如，记忆扩展存储：
- `MEMORY.md` - 干净的 markdown 文件，包含持久化记忆
- `contexts/` - 从历史会话中提取的上下文文件
- `processed_sessions.json` - 跟踪已处理的会话
