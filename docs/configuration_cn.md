...
WASM 扩展在 **`extension_manifest()` 返回的 JSON** 中声明所需宿主能力（`capabilities` 字段，snake_case）；未声明的敏感能力不会注入运行上下文。

`extension.json` 须声明 **s5r 协议版本** 与 WASM 路径；能力与工具列表由 guest 在握手时提供：
