...
| `ProcessSpawn` | `process_spawn` | （预留，未实现） |
| `NetworkClient` | `network_client` | （预留，未实现） |
...
Continuations 在宿主 `extension_call` 返回后**顺序**调度，深度上限 **16**。
...
当前宿主对 `astrcode.llm.small_chat` 在流式路径上收集 delta 后以 Event 序列返回；guest 可同样对流式 handler 响应。
...
| `astrcode.workspace.read` | `workspace_read` | 读工作区文件（路径校验） |
