...
- **共享原语**:不要创建无明确领域归属的 `support`/`utils` 桶。产品级默认目录在 `astrcode-core::config::defaults`(进程级路径原语实体在 `astrcode-paths`,由 defaults re-export);扩展可用的路径、frontmatter、shell 与 discovery 能力在 `astrcode-core`(SDK 保留 re-export);S5R 子进程运行时(peer 状态机与 I/O driver)在 `astrcode-s5r-runtime`,host 与 worker 共同依赖,不属于 SDK 作者面;工作区沙箱、展示格式化、哈希和运行时策略留在所属 crate,日志在 `astrcode-log`。写新函数前先按语义在 `crates/<owner>/src` 与调用方中搜索签名。重新实现工作区已有的原语,或手写 `std`/`tokio` 已提供的功能,属于 review 意见而非风格偏好。
