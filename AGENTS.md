...
- **工具函数**:路径/文本/哈希/frontmatter/shell 等通用工具集中在 `crates/astrcode-support`(如 `hash.rs`、`text.rs`、`hostpaths.rs`、`shell.rs`),日志在 `crates/astrcode-log`。写新函数前先按文件名映射操作,再 `rg -i 'fn \w*<term>' crates/astrcode-support/src crates/<touched>/src` 查签名。重新实现工作区已有的工具函数,或手写 `std`/`tokio` 已提供的功能,属于 review 意见而非风格偏好。
...
- 只有测试能放 `.unwrap()`。
- 遵循 SOLID:职责单一、trait/enum 组合扩展、行为契约、接口隔离、依赖抽象。
